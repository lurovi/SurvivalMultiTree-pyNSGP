...
def scatter_plot(
    ...
    fastplot.plot(
        None, f'pareto_{dataset}_seed{seed}.pdf',
        mode='callback',
...
if __name__ == '__main__':
    ...
    for dataset_name in ['whas500', 'gbsg2', 'veterans_lung_cancer']:
        for seed in range(1, 5 + 1):
            ...
            coxnet_errors_list = list(coxd['TestError'])
            ...
            nsgp_errors_list = [float(val) for val in nsgpd.loc[num_gen - 1, 'TestParetoObj1'].split(' ')]

            scatter_plot(dataset=dataset_name, seed=seed, coxnet_n_features_list=coxnet_n_features_list, coxnet_errors_list=coxnet_errors_list, nsgp_n_features_list=nsgp_n_features_list, nsgp_errors_list=nsgp_errors_list)
